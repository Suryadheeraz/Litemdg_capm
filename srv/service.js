...
const { CallEntity, Move_To_Dummy, Rules_Derivation, Rules_default, Rules_validation, createWorkflowInstance, createWorkflowCustomTaskUI, generateCUID } = require('./functions')
...
module.exports = class Service extends cds.ApplicationService {
  init() {
    ...
    const {
      ...
      ClassAssignment_dummy,
      CharacteristicValuation_dummy ,
      Storage_Location,
      Description,
      Change_Request,
    ...
    const srv = this;
      ...
      async function fetchData() {
        const response = await axios.get("https://py_mdg_duplicate_chk.cfapps.eu10-004.hana.ondemand.com/result")
        return response.data
      }


      const weights = { matnr: 0, makt_maktx: 60, mtart: 25, matkl: 15 }
      const totalWeight = Object.values(weights).reduce((a, b) => a + b, 0)
      const normalizedWeights = {}
      for (const key in weights) normalizedWeights[key] = weights[key] / totalWeight


      function computeScore(record) {
        let score = 0
        for (const field in normalizedWeights) {
          if (record[field] && String(record[field]).trim() !== '') score += normalizedWeights[field]
        }
        return score
      }


      function groupRecordsByCluster(records) {
        const groups = {}
        for (const item of records) {
          const clusterId = item.Cluster || 'NO_CLUSTER'
          if (!groups[clusterId]) groups[clusterId] = []
          groups[clusterId].push(item)
        }
        return groups
      }

      function pickBest(records, field) {
        const fieldrec = records.filter(r => r[field] && String(r[field]).trim() !== '')
        if (fieldrec.length > 0) {
          fieldrec.sort((a, b) => b._score - a._score)
          return fieldrec[0][field]
        }
        return null
      }


      this.on('READ', Allrecords, async () => {
        const apiData = await fetchData()
        return apiData.map(item => ({
          Group_id: item.Cluster,
          MATNR: item.matnr,
          MAKT_MAKTX: item.makt_maktx,
          MTART: item.mtart,
          MATKL: item.matkl
        }))
      })
      ...
      this.on('Bestrec', async () => {
        const apiData = await fetchData()
        if (!apiData || apiData.length === 0) return "No data found from API"

        const groups = groupRecordsByCluster(apiData)
        const bestRecords = []

        for (const clusterId in groups) {
          const records = groups[clusterId]
          for (const r of records) r._score = computeScore(r)
          ...
          bestRecords.push({
            Group_id: clusterId,
            MATNR: pickBest(records, 'matnr'),
            MAKT_MAKTX: pickBest(records, 'makt_maktx'),
            MTART: pickBest(records, 'mtart'),
            MATKL: pickBest(records, 'matkl')
          })
        }
        await cds.run(DELETE.from(Bestrecords))

        await cds.run(INSERT.into(Bestrecords).entries(bestRecords))
        return bestRecords
      })


      this.on('Goldenrec', async () => {
        const apiData = await fetchData()
        if (!apiData || apiData.length === 0) return "No data found from API"
        ...
        await cds.run(DELETE.from(Goldenrecords))

        await cds.run(INSERT.into(Goldenrecords).entries(goldenRecords))
    ...
    // Call action for Quality Mnagement Update 
    // this.on('updateQualityMgmt', async (req) => {

    //     const tx = cds.transaction(req);

    //     try {

    //       const items = req.data.ip_qualityMng;

    //       for (const qm of items) {

    //         const exists = await tx.read(QualityManagement)
    //           .where({
    //             parent_mat_plant_ID: qm.parent_mat_plant_ID,
    //             parent_mat_plant_MATNR: qm.parent_mat_plant_MATNR,
    //             parent_WERKS: qm.parent_WERKS
    //           });

    //         if (exists.length > 0) {
    //           // --- UPDATE ---
    //           await tx.update("litemdg.QualityManagement")
    //             .set({
    //               KZDKZ: qm.KZDKZ,
    //               PRFRQ: qm.PRFRQ,
    //               QMPUR: qm.QMPUR,
    ...
    //         } else {
    //           // --- INSERT ---
    //           await tx.insert("litemdg.QualityManagement").entries({
    //             parent_mat_plant_ID: qm.parent_mat_plant_ID,
    //             parent_mat_plant_MATNR: qm.parent_mat_plant_MATNR,
    //             parent_WERKS: qm.parent_WERKS,
    ...
    //     } catch (error) {
    //       console.error("UpdateQualityMgmt Error:", error);
    //       await tx.rollback();
    //       req.error(500, "Failed to update Quality Management");
    //     }
    //   });
    ...
    this.on("updateQualityMgmt", async (req) => {
      ...
      try {
        const list = req.data.ip_qualityMng;

        for (const qm of list) {

          // 1️⃣ READ the existing Plant + QM
          const plantRow = await tx.read(plant, {
            mat_plant_ID: qm.mat_plant_ID,
            mat_plant_MATNR: qm.mat_plant_MATNR,
            WERKS: qm.WERKS

          });

          if (!plantRow) {
            req.error(404, "Plant not found. Cannot update QM.");
          }

          // Prepare merged QM object
          const qmData = {
          ...
          // // 2️⃣ Merge QM into deep structure
          // const updatePayload = {
          //   to_QualityManagement: [qmData]
          // };

          // 3️⃣ Deep UPDATE on Plant
          await tx.run(
            UPDATE(plant)
              .set(qmData)
              .where({
        ...
        req.error(500, "Failed to update QualityManagement");
      }
    });

    // Update Workschedule 

    this.on("updateWorkSchedule", async (req) => {
      const tx = cds.transaction(req);

      try {
        const list = req.data.ip_wrkSchedule;
        ...
        for (const qm of list) {
          ...
          const qmData = {
            ...
            BEARZ: qm.BEARZ,
            DZEIT: qm.DZEIT,
            FEVMARCWR: qm.FEVMARCWR,
            FRTME: qm.FRTME,
            RUEZT: qm.RUEZT,
            SFCMARCWR: qm.SFCMARCWR,
            TRANZ: qm.TRANZ,
            UEETK: qm.UEETK,
            UEETO: qm.UEETO,
            UNETO: qm.UNETO,
        ...
        await tx.rollback();
        console.error("Deep Update Error:", err);
        req.error(500, "Failed to update WorkScheduling");
      }
    });
    ...
    srv.before(["CREATE"], "Mara.drafts", async (req) => {
      ...
      console.log("Logged-in user role : " + JSON.stringify(req.user));
      ...
      if (!existingRecord && !existingRecord_dummy) {
        ...
        if (req.user.is("mm_WorkSchedulingRole")) {
          roles.push("WorkSchedulingRole");
        }

        if (req.user.is("mm_QualityRole")) {
          roles.push("QualityRole");
        }
        ...
        req.data.RoleAssinged = "|" + roles.join("|") + "|";

        console.log("Stored Roles:", req.data.RoleAssinged);
      }
      else {
    ...
    srv.after(["UPDATE"], "Mara.drafts", async (data, req) => {
      const { ID, MAKT_MAKTX, MATNR } = data;
      if (MAKT_MAKTX || MAKT_MAKTX === null) {
        const draft_UUID = await SELECT.from("litemdg.Mara.drafts").where({
          ID: ID,
        });
         let lang = req.query['sap-language'] || req.headers['sap-language'] 
           || req.headers['accept-language'] || 'EN';

           lang = lang.split(/[-_]/)[0].toUpperCase();
        ...
        const existingEntry = await SELECT.one
          ...
          .where({ Material_ID: ID, Material_MATNR: MATNR, code: lang });
  ...
  timestamp.getSeconds().toString().padStart(2, "0");
      ...
      if (req.event === "CREATE") {
        if (flag.CREATION_TYPE == "SINGLE") {
          ...
          const result = await createWorkflowCustomTaskUI(
            req.data,
            req.user.attr.email,
            wfCreationTime,
    ...
    this.on("ProcessExcel", async (req) => {
      const { ip_mara, ip_plant, ip_storage, ip_description, ip_warehouse,
        ...
        ip_alternate_uom,
        ip_sales_delivery,
        ip_valuation, ip_sales_tax, ip_type, ip_Entity,
      ip_QualityManagement, ip_Worksheduling,ip_ClassAssignment, ip_CharacteristicValuation } = req.data;
...
// QualityManagement
          await Promise.all(
            ip_QualityManagement.map(async (entry) => {
              ...
              if (plant) {
                ...
                UPDATE(Plant_dummy)
                    .set(entry)
                    .where({
                      mat_plant_ID: plant.mat_plant_ID
                    })
                  );
              }
            })
          ...
          await Promise.all(
            ip_Worksheduling.map(async (entry) => {
               const plant = await tx.run(
                SELECT.one
                  .from(Plant_dummy)
                  .where({
          ...
          // classAssignemnt
          await Promise.all(
            ip_ClassAssignment.map(async (entry) => {
              const material = await tx.run(
                SELECT.one.from(mara_dummy).where({ MATNR: entry.MARA_MATNR })
              );
              if (material) {
                await tx.run(
                  INSERT.into(ClassAssignment_dummy).entries({
                    ...entry,
                    MARA_ID: material.ID,
                  })
                );
          ...
          await Promise.all(
            ip_CharacteristicValuation.map(async (entry) => {
              const classAssignment = await tx.run(
                SELECT.one.from(ClassAssignment_dummy).where({ MARA_MATNR: entry.CValuation_MARA_MATNR })
              );
              if (classAssignment) {
                await tx.run(
                  INSERT.into(CharacteristicValuation_dummy).entries({
                    ...entry,
                    CValuation_MARA_ID: classAssignment.MARA_ID,
                  })
                );
              }
            })
    ...
    "definitionId": "eu10.bgsw-sdsc-coe-at1drpz8.customtaskuiprocess.massuploadProcess",
    "context": {
        "changereqnumber": req_no.toString(),
        "requested_by": req.user.attr.email,
        "creation_date": creationDate,
        "creation_time": creationTime,
        "request_status": "To Be Approved",
        "description": desc,
        "approverid": "anupam.alok@in.bosch.com"
    }
        ...
        if (ip_type === 'CREATE') {
          if (changeRequestDetails.length > 0) {
            for (const detail of changeRequestDetails) {
              ...
              const classAssignemntDummyData = await tx.run(
                SELECT.from(ClassAssignment_dummy).where({
                  MARA_ID: maraDummyData[0].ID,
                })
              );

              const characteristicValuationDummyData = await tx.run(
                SELECT.from(CharacteristicValuation_dummy).where({
                  CValuation_MARA_ID: maraDummyData[0].ID,
                })
              );
              ...
              let classAssignemntInserted = true;
              let characteristicvaluationinserted = true;
              ...
              if (classAssignemntDummyData.length > 0 && maraInserted) {
                for (const entry of classAssignemntDummyData) {
                  const result = await tx.run(INSERT.into(ClassAssignment).entries(entry));
                  if (result.results.length != 1) {
                    classAssignemntInserted = false;
                    output = "Insertion Failed";
                    break;
                  }
                }
              ...
              if (characteristicValuationDummyData.length > 0 && maraInserted) {
                for (const entry of characteristicValuationDummyData) {
                  const result = await tx.run(INSERT.into(CharacteristicValuation).entries(entry));
                  if (result.results.length != 1) {
                    characteristicvaluationinserted = false;
                    output = "Insertion Failed";
                    break;
                  }
                }
    ...
    this.on("FieldStatus", async (req) => {
      ...
      this.on("CopyPlant", async (req) => {
      const { ip_RefrencePlant, ip_NewPlant, ip_Material } = req.data;

      const tx = cds.transaction(req);

      // const materialdata = await SELECT.from(Mara).where({ MATNR: ip_Material });

      const referencePlantData = await tx.run(SELECT.from(plant).where({ mat_plant_MATNR: ip_Material, WERKS: ip_RefrencePlant }));
      const referenceDraftPlant = await tx.run(SELECT.from('litemdg.Plant.drafts').where({ mat_plant_MATNR: ip_Material, WERKS: ip_RefrencePlant }));
      const draft_data = {
      ...
      const sSameplantDraftData = await tx.run(SELECT.from('litemdg.Plant.drafts').where({ mat_plant_MATNR: ip_Material, WERKS: ip_NewPlant }));
      if (sSameplantDraftData.length > 0) {
        req.error({
      ...
      const plantDraftData = await tx.run(SELECT.from('litemdg.Plant.drafts').where({ mat_plant_MATNR: ip_Material }));


      if (plantDraftData.length > 0) {
        draft_data.draftuuid = plantDraftData[0].draftadministrativedata_draftuuid
      }
      ...
      const plantData = referencePlantData.length === 0 ? referenceDraftPlant : referencePlantData;
      if (plantData.length) {


        const normalizedPlantData = plantData.map(normalizeKeysToLower);

        const updatedPlantData = normalizedPlantData.map(({ mat_plant_matnr, ...rest }) => ({
          ...rest,
          draftadministrativedata_draftuuid: draft_data.draftuuid,
          mat_plant_matnr: ip_Material,
          werks: ip_NewPlant
        }));
        ...
        const storageOrgData = await tx.run(SELECT.from(Storage_Location).where({ plant_mat_plant_MATNR: ip_Material, plant_WERKS: ip_RefrencePlant }));
        const storageDraftData = await tx.run(SELECT.from('litemdg.storage.location.drafts').where({ plant_mat_plant_MATNR: ip_Material, plant_WERKS: ip_RefrencePlant }));
        const storageData = storageOrgData.length === 0 ? storageDraftData : storageOrgData;

        const normalizedStorageData = storageData.map(normalizeKeysToLower);

        if (storageData.length) {
          const updatedStorageData = normalizedStorageData.map(({ plant_mat_plant_MATNR, ...rest }) => ({
            ...rest,

            draftadministrativedata_draftuuid: draft_data.draftuuid,
            plant_werks: ip_NewPlant
          }));
    ...
    this.on("InsertMaterial", async (req) => {
      ...
      try {
        ...
        materialData.HasActiveEntity = false,
        ...
        materialData.RoleAssinged = sRoles;
        await tx.run(
        ...
        const aAlternateUOM = await tx.run(SELECT.from(Alternate_UOM).where({Material_ID : ip_Matnr}));
        const aClassiAssignment =  await tx.run(SELECT.from(ClassAssignment).where({MARA_ID : ip_Matnr}));
        const aClassValuation =  await tx.run(SELECT.from(CharacteristicValuation).where({CValuation_MARA_ID : ip_Matnr}));
        const aSalesTax =  await tx.run(SELECT.from(Sales_tax).where({VKORG_Material_ID : ip_Matnr}));
        ...
        if (salesDeliveryData.length) {
         ...
         if (aSalesTax.length) {
          const updatedaSalesTax = aSalesTax.map(({ VKORG_Material_ID, ...rest }) => ({
            ...rest,
            VKORG_Material_MATNR: ip_NewMatnr,
            VKORG_Material_ID: gen_ID,
            draftadministrativedata_draftuuid: draft_data.draftuuid
          }));

          await tx.run(INSERT.into('litemdg.sales.tax.drafts').entries(updatedaSalesTax));
        }
        ...
        if (aAlternateUOM.length) {
          const updatedaAlternateUOM = aAlternateUOM.map(({ Material_ID, ...rest }) => ({
            ...rest,
            Material_ID: gen_ID,
            Material_MATNR: ip_NewMatnr,
            draftadministrativedata_draftuuid: draft_data.draftuuid
          ...
          await tx.run(INSERT.into('litemdg.alternate.uom.drafts').entries(updatedaAlternateUOM));
        }

        if (aClassiAssignment.length) {
          const updatedClassiAssignment = aClassiAssignment.map(({ MARA_ID, ...rest }) => ({
            ...rest,
            MARA_ID: gen_ID,
            MARA_MATNR: ip_NewMatnr,
            draftadministrativedata_draftuuid: draft_data.draftuuid
          }));

          await tx.run(INSERT.into('litemdg.classassignment.drafts').entries(updatedClassiAssignment));
        }
    

        if (aClassValuation.length) {
          const updatedaClassValuationt = aClassValuation.map(({ CValuation_MARA_ID, ...rest }) => ({
            ...rest,
            CValuation_MARA_ID: gen_ID,
            CValuation_MARA_MATNR: ip_NewMatnr,
            draftadministrativedata_draftuuid: draft_data.draftuuid
          }));

          await tx.run(INSERT.into('litemdg.characteristicvaluation.drafts').entries(updatedaClassValuationt));
        }
    ...
    this.on("Rule_validation", async (req) => {
      ...
      const ID = ip_ID
      let modifiedCondition = "";
      let error_msg, default_value,targetValue;
      let error_array = [];
      var data;
        ...
        for (const ruleHeader of ruleHeaders) {
          ...
          if (ruleLineItems.length > 0) {
            for (const rule of ruleLineItems) {
              ...
              let query, fieldKey, fieldValue;
              error_msg = errorMessage;
              default_value = defaultValue;

              if (modelTable === "MARA") {

                query = await SELECT.from("litemdg.mara.drafts")
                  .columns(modelTableField)
                  .where({ id: ID });
                data = await SELECT.from("litemdg.mara.drafts")
                  .where({ id: ID });

                let lowerCasefieldKey = modelTableField.toLowerCase();
                let upperCasefieldKey = modelTableField.toUpperCase();
                fieldValue = query[0] ? query[0][lowerCasefieldKey] : null;
                if (!fieldValue) {
                  fieldValue = query[0] ? query[0][upperCasefieldKey] : null;
                }
              } else if (modelTable === "MARC") {
                query = await SELECT.from("litemdg.plant.drafts")
                  .columns(modelTableField)
                  .where({ mat_plant_ID: ID });
          ...
          if (conditionMet && ruleHeader.isMandatory === false) {
           ...
           targetValue = data[0][ruleHeader.targetField];
            targetValue = targetValue === undefined ? data[0][ruleHeader.targetField.toLowerCase()] : data[0][ruleHeader.targetField];
            const targetType = typeof targetValue;
           ...
           targetValue = data[0][ruleHeader.targetField];
            ...
            if (targetValue === null || targetValue === "0.000" || targetValue === undefined) { // Corrected equality checks
              req.error({
                code: 'Validation Error',
                message: error_msg,
                status: 418
    ...
    this.on("Mass_Rule_validation", async (req) => {
      ...
      for (const ID of ip_ID) {
        for (const ruleHeader of ruleHeaders) {
          ...
          var error_msg_temp = ID + ": " + error_msg
          if (conditionMet && ruleHeader.isMandatory === false) {
            ...
            if (!normalizedDefaults.includes(normalizeValue(targetValue, targetType))) {
              validateErrors.push({
                matnr: ID,
                model: ruleHeader.model,
                tableEntityDesc: ruleHeader.tableEntityDesc,
                targetFieldDesc: ruleHeader.targetFieldDesc + ' (' + ruleHeader.targetField + ')',
                ruleName: ruleHeader.ruleName === null ? ruleHeader.ruleType : ruleHeader.ruleName,
                errors: [
                  {
                    message: error_msg,
                  },
            ...
            targetValue = data[0][ruleHeader.targetField];
            targetValue = targetValue === undefined ? data[0][ruleHeader.targetField.toLowerCase()] : data[0][ruleHeader.targetField];
            if (targetValue === '' || targetValue === "0.000" || targetValue === null) { // Corrected equality checks
              validateErrors.push({
                matnr: ID,
                model: ruleHeader.model,
                tableEntityDesc: ruleHeader.tableEntityDesc,
      ...
      req.reply({ validation_errors: validateErrors });
        ...
        this.on("ValidationReport", async (req) => {
      const { RulesHeader, RuleLineItems, } = srv.entities;
      const ip_ID = await SELECT.from(Mara)
        .where({ MTART: req.data.ip_MTART });
      ...
      for (const ipMatnr of ip_ID) {
        let sMATNR = ipMatnr.MATNR;
        let sMTART = ipMatnr.MTART;

        const ip_Validrepo = await SELECT.from("litemdg.ValidationReportHeader")
        .where({ MTART: req.data.ip_MTART, MATNR :sMATNR});

        if(ip_Validrepo.length > 0){
         await DELETE.from("litemdg.ValidationReportHeader").where({ MTART: req.data.ip_MTART,MATNR :sMATNR });
        }
        for (const ruleHeader of ruleHeaders) {
          const ruleLineItems = await SELECT.from(RuleLineItems).where({
            ruleHeader_ID: ruleHeader.ID,
          ...
          for (const rule of ruleLineItems) {
            ...
            if (modelTable === "MARA") {
              const query = await SELECT.from(Mara)
                .columns(modelTableField)
                .where({ MATNR: sMATNR });
              data = await SELECT.from(Mara)
                .where({ MATNR: sMATNR });
              ...
              const query = await SELECT.from(plant)
                .columns(modelTableField)
                .where({ mat_plant_MATNR: sMATNR });

              data = await SELECT.from(plant).where({ mat_plant_MATNR: sMATNR });
          ...
          var error_msg_temp = sMATNR + ": " + error_msg
          if (conditionMet && ruleHeader.isMandatory === false) {
           ...
           targetValue = data[0][ruleHeader.targetField];
            ...
            if (!normalizedDefaults.includes(normalizeValue(targetValue, targetType))) {
              validateErrors.push({
                ValidationId: sMTART + '_' + getTimestampString(),
                MATNR: sMATNR,
                MTART: sMTART,
                model: ruleHeader.model,
                tableEntity: ruleHeader.tableEntityDesc,
                targetField: ruleHeader.targetFieldDesc + ' (' + ruleHeader.targetField + ')',
                ruleName: ruleHeader.ruleName === null ? ruleHeader.ruleType : ruleHeader.ruleName,
      ...
      await INSERT.into("litemdg.ValidationReportHeader").entries(validateErrors);
      req.reply({ validation_errors: validateErrors });
